...
object PostMongoCodec:
  ...
  private case class MongoPost(_id: UUID, author: UUID, createdAt: Instant, imageUrl: String, message: Option[String]):
    // this shouldn't throw but zio-json doesn't seem to allow propagating custom errors for map/contramap
    def toPost: Post = Post(PostId(_id), UserId(author), createdAt, HttpsUrl.createUnsafe(imageUrl), message)
  private object MongoPost:
    def fromPost(user: Post): MongoPost =
      MongoPost(user.id.value, user.author.value, user.createdAt, user.imageUrl.value.toExternalForm, user.message)
